...
| Variable | Required | Description |
|----------|----------|-------------|
| `REDIS_URL` | No | Enables Redis-first question serving. Accepts any Redis-compatible URL: `rediss://default:TOKEN@xxx.upstash.io:6379` (Upstash over TLS) or `redis://localhost:6379` (local Redis). Toggle providers by changing this single value. Each queue key is `technology:difficulty:jobTitle`, a sorted set scored by epoch timestamp; questions are served oldest-first via `ZPOPMIN`. When the queue has enough questions no model is called. On shortage or an unreachable database, the shortfall is generated through the configured mode above. Redis-sourced questions that are abandoned (the quiz page is unloaded before the assessment is finished) are returned to their keys via `ZADD` and the in-memory session is deleted. |
